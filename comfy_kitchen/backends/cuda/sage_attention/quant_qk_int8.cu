...
//   Q path: warp-per-group, single-pass, vectorized float2 loads / int32 stores.
//   K path: warp-per-group, double-read (L1-cached), vectorized loads / stores.
...
__device__ __forceinline__ void store4_i8(int8_t* ptr,
                                          int8_t a, int8_t b,
                                          int8_t c, int8_t d)
{
        ...
        if (ch + 3 < C) {
            ...
            for (int j = 0; j < NR; ++j) {
                ...
                if (n < L) {
                    ...
                    mx = fmaxf(mx, fmaxf(
                        fmaxf(fabsf(v[j*4]),   fabsf(v[j*4+1])),
            ...
            for (int j = 0; j < NR; ++j) {
                ...
                if (n < L) {
                    store4_i8(&out[(int64_t)n * C + ch],
                              quant_int8(v[j*4],   sc),
                              quant_int8(v[j*4+1], sc),
                              quant_int8(v[j*4+2], sc),
...
__device__ void process_k(
    const T* __restrict__ in,
    int8_t*  __restrict__ out,
    float*   __restrict__ sc_buf,
    const int oblk,
    const int L, const int C,
    const int BLKK, const int WARPK)
{
    const int lane = threadIdx.x & 31;
    const int wid  = threadIdx.x >> 5;
    const int ch   = lane << 2;
        ...
        for (int j = 0; j < NL; ++j) {
            ...
            for (int p = 0; p < 2; ++p) {
                ...
                if (n < L) {
                    store4_i8(&out[(int64_t)n * C + ch],
                              quant_int8(v[vi],   sc),
                              quant_int8(v[vi+1], sc),
                              quant_int8(v[vi+2], sc),
...
void quant_qk_fused(
    ...
    const int Lq, const int Lk, const int C,
    const int BLKQ, const int WARPQ, const int BLKK, const int WARPK,
    const int q_oblk_count, const int H_q, const int H_kv,
    const int q_sc_per_h, const int k_sc_per_h)
{
    ...
    if (blockIdx.x < (unsigned)q_oblk_count) {
        ...
        process_q<T, NR>(q_in + bh, q_out + bh, q_sb + sbh,
                          blockIdx.x, Lq, C, BLKQ, WARPQ);
    } else {
        if (h >= H_kv) return;
...
#define DO(T)                                   \
    switch (nr * 100 + nl) {                    \
        case 402: DISPATCH(T, 4, 2); break;     \
        case 404: DISPATCH(T, 4, 4); break;     \
        case 408: DISPATCH(T, 4, 8); break;     \
        case 208: DISPATCH(T, 2, 8); break;     \
        case 808: DISPATCH(T, 8, 8); break;     \
        default:  DISPATCH(T, 4, 8); break;     \
    }
